/* BLM NOTE FOR THIS CHANGE */
/*
we are adding academic period to the following: shrachg, shrcchg, shrdchg, sprpchg, ssrochg
we are doing this only for chg_student_course_attr right now 
there may be a newer version of this file out there, so pay only attention to those changes */
...
--
  CASE
    ...
    WHEN p_chg_table = 'SHRACHG' THEN
      ...
      --BLM modifying 12/14/15 to add academic period to this change table
      ...
      EXECUTE IMMEDIATE(f_prepare_sql('CHG_STUDENT_COURSE_ATTR',      'edw_chg.PERSON_UID = src.SHRACHG_PIDM and edw_chg.academic_period = src.shrachg_term_code) AND src.SHRACHG_PROCESS_ID = ''', 'src.SHRACHG_PIDM, NULL, '''|| p_chg_table || ''', SYSDATE, src.shrachg_term_code'));
    WHEN p_chg_table = 'SHRCCHG' THEN
      ...
      --BLM modifying 2/14/15 to add academic_period to this change table
      -- EXECUTE IMMEDIATE(f_prepare_sql('CHG_STUDENT_COURSE_ATTR',      'edw_chg.PERSON_UID = src.SHRCCHG_PIDM) AND src.SHRCCHG_PROCESS_ID = ''', 'src.SHRCCHG_PIDM, NULL, '''|| p_chg_table || ''', SYSDATE'));
       EXECUTE IMMEDIATE(f_prepare_sql('CHG_STUDENT_COURSE_ATTR',      'edw_chg.PERSON_UID = src.SHRCCHG_PIDM and edw_chg.academic_period = src.shrcchg_term_code) AND src.SHRCCHG_PROCESS_ID = ''', 'src.SHRCCHG_PIDM, NULL, '''|| p_chg_table || ''', SYSDATE, src.shrcchg_term_code'));
    WHEN p_chg_table = 'SHRDCHG' THEN
     ...
     EXECUTE IMMEDIATE(f_prepare_sql('CHG_ACADEMIC_OUTCOME',         'edw_chg.PERSON_UID = src.SHRDCHG_PIDM) AND src.SHRDCHG_PROCESS_ID = ''', 'src.SHRDCHG_PIDM, '''|| p_chg_table || ''', SYSDATE'));
      ...
      --BLM modifying 12/14/15 to add NULL academic_period to this change table
      ...
      EXECUTE IMMEDIATE(f_prepare_sql('CHG_STUDENT_COURSE_ATTR',      'edw_chg.PERSON_UID = src.SHRDCHG_PIDM) AND src.SHRDCHG_PROCESS_ID = ''', 'src.SHRDCHG_PIDM, NULL, '''|| p_chg_table || ''', SYSDATE, NULL'));
    ...
    WHEN p_chg_table = 'SPRPCHG' THEN
     ...
     --10/2/14 BLM hard coding in change table names
      ...
      --10/3/14 BLM - hard coding in change table names
      --12/14/15 BLM  - putting in NULL for custom academic period field
      ...
      EXECUTE IMMEDIATE(f_prepare_sql('CHG_STUDENT_COURSE_ATTR',      'edw_chg.PERSON_UID = src.SPRPCHG_PIDM) and src.sprpchg_table_name in (''PREVIOUS_EDUCATION_PCOL'',''ACADEMIC_OUTCOME'',''MST_RACE'',''PERSON'',''MEDICAL_INFORMATION'',''VISA'',''GENERAL_STUDENT'')  AND src.SPRPCHG_PROCESS_ID = ''', 'src.SPRPCHG_PIDM, NULL, '''|| p_chg_table || ''', SYSDATE, NULL'));
    ...
    WHEN p_chg_table = 'SSROCHG' THEN
     --10/3/14 BLM modifying to include only proper change records
      ...
      --BLM modifying 12/14/15 to add academic period to this change table
      EXECUTE IMMEDIATE(f_prepare_sql('CHG_STUDENT_COURSE_ATTR',      'edw_chg.COURSE_REFERENCE_NUMBER = src.SSROCHG_CRN and edw_chg.academic_period = src.ssrochg_term_code) AND src.SSROCHG_PROCESS_ID = ''', 'NULL, src.SSROCHG_CRN, '''|| p_chg_table || ''', SYSDATE, src.ssrochg_term_code'));
