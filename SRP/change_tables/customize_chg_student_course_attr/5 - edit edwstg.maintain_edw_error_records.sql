...
--
  ...
  BEGIN
    ...
    WHEN (p_column = 'PERSON_UID' AND p_column2 IS NULL) THEN
       ...
       END IF;
    /*** 1/14/2015 BLM added to support chg_student_course and chg_student_course_attr additional field we added - academic_period */
    WHEN (p_column = 'PERSON_UID' AND p_column2 IS NOT NULL and p_chg_table like 'CHG_STUDENT_COURSE%') THEN
       ret_statement := 'INSERT INTO '|| P_CHG_TABLE ||' tgt ' ||
   ...
   --NOTE, added in "p_change_table not like 'CHG_STUDENT_COURSE%'", but I do not think it is necessary
   --edited on 12/14/15, changed from "p_change_table <> 'CHG_STUDENT_COURSE'"  to "p_change_table not like 'CHG_STUDENT_COURSE%'"
    WHEN (p_column = 'PERSON_UID' AND p_column2 IS NOT NULL  and p_chg_table not like 'CHG_STUDENT_COURSE%') THEN
       ret_statement := 'INSERT INTO '|| P_CHG_TABLE ||' tgt ' ||
